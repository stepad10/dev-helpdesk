...
describe('System Prompt Builder', () => {
  ...
  it('should serialize knowledge documents correctly', () => {
    const prompt = buildSystemPrompt(mockKb);
    expect(prompt).toContain('[DOC-999] Topic: Test Topic | Status: current | Last updated: 2025-01-01\nContent: This is a test content block.');
  });

  it('should format normal experts with their handle', () => {
    const prompt = buildSystemPrompt(mockKb);
    expect(prompt).toContain('Name: Test Setup | Handle: @test_handle | Skills: testing | Availability: available');
  });

  it('should handle null expert handles without leaking the word "null"', () => {
    const prompt = buildSystemPrompt(mockKb);
    expect(prompt).toContain('Name: Null Handle Expert | Handle: no handle — contact via team lead | Skills: edge | Availability: on_leave');
